...
void test_piece_etre_integre()
{
    ...
    piece_siam* piece=NULL;//malloc(sizeof(piece_siam));
    piece->type=elephant; // à modifier pour tester tous les cas
    piece->orientation=haut;
    if(piece_etre_integre(piece)==1) puts("piece integre");
    else puts("piece non-integre");
...
void test_plateau_modification_changer_orientation_piece_etre_possible()
{
    ...
    if(plateau_modification_changer_orientation_piece_etre_possible(&plateau, 2, 3, gauche)!=1)

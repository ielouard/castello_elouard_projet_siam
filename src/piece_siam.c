...
int piece_etre_integre(const piece_siam* piece)
{
    ...
    type=type_etre_animal(piece->type);
    orientation=orientation_etre_integre_deplacement(piece->orientation);
...
void piece_definir(piece_siam* piece,type_piece type,orientation_deplacement orientation)
{
    ...
    if(type_etre_animal(type)==1 && orientation_etre_integre_deplacement(orientation))
    {
        piece->type=type;
